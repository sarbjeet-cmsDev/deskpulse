...
export class PerformanceService {
  ...
  async createPerformance(TaskObj: any) {
    ...
    const taskTypeWeight = this.TaskTypeWeightage.find(t => t.taskType === taskType)?.weight || 0;
    const taskStatusWeight = this.taskStatusTypeWeightage.find(t => t.taskStatus === taskStatus)?.weight || 0;
    const priorityWeight = this.PriorityWeightage.find(t => t.PriorityType === priorityType)?.weight || 0;
    const acceptanceWeight = this.AcceptanceWeightage.find(t => t.AcceptanceType === acceptanceType)?.weight || 0;
    const revisionWeight = this.RevisionWeightage.find(t =>
    ...
    const taskStatusScore = (taskStatusWeight / taskStatusMax) * taskStatusPercent;

...
import { checkUserExists, comparePassword, hashPassword } from "../user/user.helper";
...
export class AuthService {
  ...
  async login({ email, password }: Partial<User>): Promise<any> {
    ...
    if (!user.isActive) {
      throw new NotFoundException('Account not verified! Please check your email and verify account.');
    }
    const payload = {
  ...
  async resetPassword(id: string, token: string, newPassword: string): Promise<string> {

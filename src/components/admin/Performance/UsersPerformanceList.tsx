...
export default function PerformancePreview() {
  ...
  async function fetchPerformance(userIds: string[]) {
    ...
    try {
      const result = await AdminPerformanceService.getAllUsersPerformance({
        userId: userIds[0],
      ...
      if (
        ...
        result.length === 0 ||
        !Array.isArray(result[0]?.performance)
      ) {
        console.warn("Unexpected performance data structure");
        return;
      }

      const performanceData = result[0].performance;

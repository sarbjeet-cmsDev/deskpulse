...
// ✅ Properly typed dynamic import for react-select
  ...
  const fetchUsers = useCallback(
    debounce(async (input: string) => {
      try {
        ...
        // ✅ Merge previously selected options (to keep them in dropdown) + new ones (from API)
        setUserOptions((prevOptions) => {
          const existingMap = new Map(prevOptions.map((opt) => [opt.value, opt]));
